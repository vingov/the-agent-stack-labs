...
usage() {
  ...
  printf 'Creates a disposable copy of the synthetic lab fixtures.\n'
}
...
if (( $# == 1 )); then
  ...
  destination="$(mktemp -d "${TMPDIR:-/tmp}/the-agent-stack-hermes-02.XXXXXX")"
fi
...
hermes_home="$destination/profile"

mkdir -p "$workspace" "$hermes_home" "$destination/evidence" "$destination/logs" "$destination/runs"
cp -R "$fixtures/workspace/." "$workspace/"
cp -R "$fixtures/profile/." "$hermes_home/"

git -C "$workspace" init --quiet
git -C "$workspace" config user.name "The Agent Stack Labs"
git -C "$workspace" config user.email "lab@example.invalid"
git -C "$workspace" add .
git -C "$workspace" commit --quiet -m "Add synthetic context fixtures"
